...
export async function analyzeCode(
  ...
  if (!USE_REAL_API) {
    ...
    console.log(
      `[Vibe-Guard] Analyzing ${language} code (MOCK MODE)`
    );
    ...
    return {
      ...
      explanation:
        "The script invokes a recursive delete via os.system('rm -rf').",
      ...
      diagnostics: [
        {
          ...
          remediation:
            'Avoid executing untrusted shell commands. Validate and constrain input before execution.'
        }
      ],
  ...
  console.log(
    `[Vibe-Guard] Analyzing ${language} code (REAL MODE)`
  );
  ...
  try {
    ...
    const extractedThreats: string[] =
      diagnostics
        .map((diagnostic: Diagnostic) =>
          diagnostic.message || 'Security issue detected.'
        )
        .filter(Boolean);
    ...
    const staticRisk =
      typeof data.staticRisk === 'number'
        ? data.staticRisk
        : 0;

    const runtimeRisk =
      typeof data.runtimeRisk === 'number'
        ? data.runtimeRisk
        : 0;

    const aiRisk =
      typeof data.aiRisk === 'number'
        ? data.aiRisk
        : 0;
    ...
    return {
      ...
      sandboxResult: {
        ...
        filesCreated:
          sandbox.filesCreated || [],

        filesModified:
          sandbox.filesModified || [],

        filesDeleted:
          sandbox.filesDeleted || [],

        processesSpawned:
          sandbox.processesSpawned || [],

        networkAttempts:
          sandbox.networkAttempts || [],
